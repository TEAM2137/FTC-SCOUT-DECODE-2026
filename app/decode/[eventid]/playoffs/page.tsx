...
export default function Page({ params, }: { params: Promise<{ eventid: string }>}){
    ...
    useEffect(() => {
        fetch(`/api/ftc-events/events/${eventid}/playoffs`)
        .then(res => res.json())
        .then(data => {
            setQualMatches(data);
        })
...
{event.code === 'loading' ? (
    ...
    <div className='text-black p-2 w-[100%]'>
        ...
        <h1 className="p-2 font-bold text-xl text-black">Playoff Matches</h1>
        <p className="p-2 font-light text-sm text-black italic mb-2">During the event the yet to be played matches will be displayed first with the results from the played matches listed below.</p>

    {qualMatches.length < 1 ? (
        <p className="p-5 bg-red-200 rounded-md text-xl font-bold mb-2 italic">No Matches to display. The playoff schedule is updated after alliance selections are complete.</p>
