...
const defaultMatchScore: IMatchScore = {
    scoutID: '',
    scoutTeam: scoutTeam,
    scoutName: scoutName,
    eventCode: '',
...
const Record = ({ eventCode, level, number, series, team }: { eventCode: string, level: string, number: number, series: number, team: number}) => {
  const [matchScore, setMatchScore] = useState<IMatchScore>(defaultMatchScore);
  const [scoutID] = useState<string>(scoutName + "_" + scoutTeam + "_" + eventCode + "_" + level + "_" + number + "_" + series + "_" + team);
  const [tournametLevel] = useState<string>(level);
  ...
  useEffect(() => {
    ...
    newMatchScore.scoutID = scoutID;
    newMatchScore.scoutTeam = scoutTeam;
    newMatchScore.scoutName = scoutName;
    newMatchScore.eventCode = eventCode;
    newMatchScore.matchLevel = tournametLevel;
    newMatchScore.matchNumber = number;
    newMatchScore.matchSeries = series;
    newMatchScore.teamNumber = team;
    ...
    newMatchScore.ignore = false;
    newMatchScore.hide = false;
...
useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-1 min-h-screen bg-slate-900 w-full h-full place-items-center justify-items-center">
        ...
        {save && (
            <div className="absolute top-25  flex flex-col p-1 m-auto w-[350px] min-h-4/6 bg-slate-800 rounded-lg z-100 ">
        ...
        <div className="mt-3 p-1 w-[350px] bg-slate-800 rounded-lg">
        ...
        <div className="flex flex-col p-1 w-[350px] bg-slate-800 rounded-lg">
            <div className="m-1 grid grid-cols-2 grid-rows-1 justify-between p-1">
                ...
                <div className="row-start-1 col-start-2 col-span-1 place-items-center m-auto" >
                  <div className="flex place-items-start justify-start p-0 rounded-full bg-gray-900 border-2 border-gray-500 w-[50px] " onClick={() => setAutoLeave(!autoLeave)}>
                    <div className="flex flex-col items-center justify-center w-5 h-5 bg-red-700 border-4 border-red-900 rounded-full">&nbsp;</div>
                  </div>
                </div>
                ) : (
                <div className="row-start-1 col-start-2 col-span-1 place-items-center m-auto" >
                  <div className="flex place-items-end justify-end p-0 rounded-full bg-gray-900 border-2 border-gray-500 w-[50px] " onClick={() => setAutoLeave(!autoLeave)}>
                    <div className="flex flex-col items-center justify-center w-5 h-5 bg-green-800 border-4 border-green-400 rounded-full">&nbsp;</div>
                  </div>
                </div>  
                )}
            </div>
        ...
        <div className="p-1 w-[350px] bg-slate-800 rounded-lg">
            <div className="grid grid-cols-3 grid-rows-1 justify-between p-1">
        ...
        <div className="p-2 w-[350px] bg-slate-800 rounded-lg">
            <div className="grid grid-cols-2 grid-rows-2 justify-between p-1">
                ...
                <div className="my-1 row-start-3 col-start-2 col-span-1 place-items-center m-auto" >
                  <div className="flex place-items-end justify-end p-0 rounded-full bg-gray-900 border-2 border-gray-500 w-[50px]  h-[25px]" onClick={() => setTeleBase('NONE')}>
                    <div className="flex flex-col items-center justify-center w-5 h-5 bg-green-800 border-4 border-green-400 rounded-full">&nbsp;</div>
                  </div>
                </div>  
                )}
        ...
        <div className="p-2 w-[350px] bg-slate-800 rounded-lg">
            <div className="grid grid-cols-2 grid-rows-2 justify-between p-1">
                ...
                <div className="my-1 row-start-3 col-start-2 col-span-1 place-items-center m-auto" >
                  <div className="flex place-items-end justify-end p-0 rounded-full bg-gray-900 border-2 border-gray-500 w-[50px]  h-[25px]" onClick={() => setDoubleBase('NONE')}>
                    <div className="flex flex-col items-center justify-center w-5 h-5 bg-green-800 border-4 border-green-400 rounded-full">&nbsp;</div>
                  </div>
                </div>  
                )}
        ...
        <div className="flex flex-col p-1 w-[350px] bg-slate-800 rounded-lg">
            <div className="grid grid-cols-2 grid-rows-1 justify-between p-1">
                ...
                <div className="row-start-2 col-start-2 col-span-1 text-white font-medium text-center" >
                <Link href={`/scout/event/${eventCode}/schedule`} >
